...
cAppli_Ortho::cAppli_Ortho(
    ...
    int argc,
    char **argv) : mWorkDir(StdWorkdDir(aCO.WorkDir(), argv[1])),
                   mCO(aCO),
                   ...
                   mNbIm2Test(0)
{
    ...
    if (mEgalise)
    {
        ...
        mCompMesEg = (!ELISE_fp::exist_file(mNameFileMesEg)) || (!mSE->UseFileMesure().Val()) || (mSE->RapelOnEgalPhys().Val());
    ...
    // ADDED BY GABRIEL
    mGenMasks = false;
    mMaskOutDir = "OrthoByFrameMasks/"; 

    for (int i = 1; i < argc; ++i) {
        std::string arg = argv[i];
        if (arg == "GenMasks=true") {
            mGenMasks = true;
        }
        std::string prefix = "MaskOutDir=";
        if (arg.rfind(prefix, 0) == 0) {
            mMaskOutDir = arg.substr(prefix.length());
            if (!mMaskOutDir.empty() && mMaskOutDir.back() != '/') {
                mMaskOutDir += '/';
            }
        }
    }
...
void cAppli_Ortho::SauvLabel()
{
    ...
    Fonc_Num aFonc = Min(254, (aSL >= 0) * mLutInd.in()[Max(0, aSL)]) + 255 * (aSL < 0);
