...
int Tawny_main(int argc, char **argv)
{
    ...
    bool mGenMasks = false;
    std::string mMaskOutDir = "data/OrthoByFrameMasks/";
    ...
    ElInitArgMain(
        ...
        LArgMain() << EAM(mDoRadiomEgal, "RadiomEgal", true, "Perform or not radiometric egalization (Def=true)")
                   ...
                   << EAM(mGenMasks, "GenMasks", true, "Generate contribution masks (Def=false)", eSAM_IsBool)
                   << EAM(mMaskOutDir, "MasqDir", true, "Output directory for masks (relative to WorkDir, Def=data/OrthoByFrameMasks/)"));
    if (!MMVisualMode)
    {
#if (ELISE_windows)
        ...
        std::string aCom = MM3dBinFile("Porto") + MMDir() + std::string("include/XML_MicMac/Param-Tawny.xml ") + std::string(" %WD=") + aDir + std::string(" +DR1X=") + ToString(mDeqXY.x) + std::string(" +DR1Y=") + ToString(mDeqXY.y) + std::string(" +DR0X=") + ToString(aDegCste.x) + std::string(" +DR0Y=") + ToString(aDegCste.y) + std::string(" +DegRapX=") + ToString(mDegRapXY.x) + std::string(" +DegRapY=") + ToString(mDegRapXY.y) + std::string(" +RapGlobPhys=") + ToString(mRapGlobPhys) + std::string(" +DynGlob=") + ToString(mDynGlob) + std::string(" +NameOrtho=") + aNameOut;

        if (mImPrio0 != "")
            aCom = aCom + " +ImPrio=" + QUOTE(mImPrio0);
        if (EAMIsInit(&mSzV))
            aCom = aCom + " +SzV=" + ToString(mSzV);
        if (EAMIsInit(&mNbPerIm))
            aCom = aCom + " +NbPerIm=" + ToString(mNbPerIm);
        if (EAMIsInit(&mCorrThresh))
            aCom = aCom + " +CorrThresh=" + ToString(mCorrThresh);

        if (!mDoL1Filter)
            aCom = aCom + " +DoL1Filter=false ";
        if (!mDoRadiomEgal)
            aCom = aCom + " +DoRadiomEgal=false ";
        if (mGenMasks){
            aCom = aCom + " +GenMasks=true";
            aCom = aCom + " +MaskOutDir=" + mMaskOutDir;
        }
